...
#ifndef ACTUAL_PROGRAM_3_LIST342_H
#define ACTUAL_PROGRAM_3_LIST342_H
...
/*
 * NOTE: remove and peek functions stops working if changed from && to || in operator== in Child class.
 * however, then, insert and merge functions 100% work and operator<< mostly works
 */
...
class List342
{
    ...
    friend ostream &operator<<(ostream &os, const List342<ItemType> &that)
    {
        if(that.isEmpty())
        {
            os << "Empty list";
        ...
        Node *thatPtr;
        thatPtr = that.head;
        while (thatPtr != nullptr) //while the list has elements
        {
            ItemType *print = thatPtr->data;
            os << *print;
            thatPtr = thatPtr->next; //traverses down the list
        }
        return os;
    }
...
public:
    ...
    List342(const List342 &that); //Copy constructor
    ~List342(); //Destructor
    ...
    //Operator Overloads
    List342<ItemType> operator+(const List342<ItemType> &that) const;
    void operator+=(List342<ItemType> &that);
    void operator=(const List342<ItemType> &that);
    bool operator==(const List342<ItemType> &that) const;
    bool operator!=(const List342<ItemType> &that) const;
...
#include "List342.cpp"
#endif //ACTUAL_PROGRAM_3_LIST342_H


//List342.cpp IMPLEMENTATION BELOW
...
bool List342<ItemType>::Insert(ItemType *obj)
{
    ...
    else //the rest of the list
    {
        ...
        if(pNode != nullptr)
        {
            pNode->next = insObj; //found position to insert
            return true;
        }
        delete insObj; //deallocate memory
        insObj = nullptr;
    }
    delete insObj; //deallocate memory
    insObj = nullptr;
